...
export default function ContactSection() {
  const phoneNumber = "+94740646427"; // Sri Lankan number, +94 is the country code
  const formattedPhoneNumber = "074 064 6427"; // For display
        ...
        <ScrollAnimationWrapper className="w-full max-w-2xl" style={{ transitionDelay: `100ms` }}>
          <div className="space-y-10 p-8 sm:p-10 bg-secondary rounded-2xl shadow-xl"> {/* This inner div provides solid background for text */}
            ...
            <div className="flex items-start gap-5 group">
              ...
              <div>
                <h4 className="font-semibold text-xl lg:text-2xl text-foreground mb-1">Email Relay</h4>
                <Link href={`mailto:${emailAddress}`} className="text-lg lg:text-xl text-muted-foreground hover:text-accent transition-colors duration-300 break-all">
                  {emailAddress}
                </Link>
              </div>
            </div>
            ...
            <div className="flex items-start gap-5 group">
              ...
              <div>
                <h4 className="font-semibold text-xl lg:text-2xl text-foreground mb-1">Direct Comms</h4>
                <Link href={`tel:${phoneNumber}`} className="block text-lg lg:text-xl text-muted-foreground hover:text-accent transition-colors duration-300">
                  {formattedPhoneNumber} (Call)
                </Link>
                <Link href={whatsappLink} target="_blank" rel="noopener noreferrer" className="block text-lg lg:text-xl text-muted-foreground hover:text-accent transition-colors duration-300">
            ...
            <div className="flex items-start gap-5 group">
              ...
              <div>
                <h4 className="font-semibold text-xl lg:text-2xl text-foreground mb-1">Our Nexus</h4>
                <p className="text-lg lg:text-xl text-muted-foreground">Polonnaruwa, Sri Lanka</p>
              </div>
            </div>
          </div>
        </ScrollAnimationWrapper>
